...
class GMVAEVisualize(BaseVisualize):
    ...
    def plot_in_grid(self, x, title, num_samples_to_plot=5 ):
        ...
        for i in range(num_samples_to_plot):
            for j in range(num_samples_to_plot):
                img = x[count, :].reshape([x.shape[1], x.shape[2], x.shape[3]])
                img = img*255
    ...
    def recons(self, x_input, x_labels, x_recons, z_recons, w_recons, y_recons, num_samples_to_plot=5):
        ...
        for i in range(num_samples_to_plot):
            img = x_input[i, :].reshape([x_input.shape[1], x_input.shape[2], x_input.shape[3]])
            img = img*255
            img = img.astype(np.uint8)
            axarr[i, 0].imshow(img, cmap='gray')
            
            img = x_recons[i, :].reshape([x_input.shape[1], x_input.shape[2], x_input.shape[3]])
            img = img*255
            img = img.astype(np.uint8)
