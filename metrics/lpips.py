...
def lpips(input0, input1, model='net-lin', net='alex', version=0.1):
    ...
    batch_shape = tf.shape(input=input0)[:-3]
    input0 = tf.reshape(input0, tf.concat([[-1], tf.shape(input=input0)[-3:]], axis=0))
    input1 = tf.reshape(input1, tf.concat([[-1], tf.shape(input=input1)[-3:]], axis=0))
